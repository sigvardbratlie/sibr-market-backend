...
from sibr_api import ApiBase,APIkeyError,RateLimitError
...
class nominatimAPI(ApiBase):
    ...
    def transform_output(self,response : tuple[str,dict] | dict) -> pd.DataFrame:
    ...
    def save_func(self,results : list[tuple[str,dict]],table_name : str = None, dataset_name : str = None):
        ...
        dfs = [self.transform_output(result) for result in results if result is not None]
...
class geonorgeAPI(ApiBase):
    ...
    def transform_output(self,response : tuple[str,dict] | dict):
        if isinstance(response, tuple):
            item_id = response[0]
            json_data = response[1]
        elif isinstance(response, dict):
